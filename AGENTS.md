...
- Published article locales are `en`, `fi`, `sv`, `nb`, `de`, `da`, `fr`, `es`, and `it`. Every available translation is paired by `sourceNumber`; detail pages emit reciprocal `hreflang` links only for routed versions, while English remains `x-default`.
- `src/data/articleLocaleConfig.mjs` is the publication gate and locale contract. It owns each locale's regional profile, URL root, article-index and search-index slugs, localized hub slugs, UI copy, tag map reference, importer source, source/preview support, and explicit `productionPublished` state. Content presence alone must never publish a locale.
- Swedish, Norwegian Bokmål, German, Danish, French, Spanish, and explicitly enabled preview locales use the shared dynamic route family under `src/pages/[locale]/[articles]/`. `src/data/articleLocales.mjs` and `src/data/articlePaths.mjs` derive their public locale and path behavior from the central contract.
- Italian `it` content is production-published through the shared localized route family. It is included in the default build, sitemap, and reciprocal hreflang groups; do not pass `RUNCHECK_PREVIEW_ARTICLE_LOCALES=it`, because published locales cannot be requested as previews.
- `src/data/articleSlugs.mjs` is the only source of localized article-slug generation. Published slug migrations and their permanent Cloudflare Pages redirects are recorded in `src/data/articleSlugMigrations.mjs` and `public/_redirects`; retain those redirects indefinitely.
...
- `src/components/Hero.astro` is a finished landing section, not a stub. `Features.astro`, `Pricing.astro`, `FAQ.astro`, and `CTA.astro` remain foundation stubs; do not turn those into final marketing design in this phase.
- The hero phone renders `public/assets/runcheck-phone-condition-hero-plate.webp`, a copy of the source image with the screen's runcheck mark removed. The live mark is inlined over it from `src/assets/runcheck-logo.svg` so its parts can be animated. Regenerate the plate with `node scripts/make-hero-plate.mjs` whenever the source image or the logo changes; the script reads `public/assets/runcheck-phone-condition-hero.webp`, which is kept solely as that source. The script also prints the `.hero-logo` placement percentages used in `global.css` — never hand-tune those, rerun the script.
- `src/assets/runcheck-logo.svg` is the only logo file. `Header.astro` and `Footer.astro` load it as an image; `Hero.astro` imports it with `?raw` and inlines it. Its mark is split into `.rc-hook` and `.rc-arrow` groups that both draw one shared `#rcPaint` layer through separate clip paths, so the two parts animate independently without duplicating the artwork.
...
- Article frontmatter schema:
  ...
  - `subgroup?: string`
  - `tags: string[]`
- `locale` is validated from the source-supported locale list in `src/data/articleLocaleConfig.mjs` (currently `en`, `fi`, `sv`, `nb`, `de`, `da`, `fr`, `es`, and `it`; defaults to `en`)
  - `draft: boolean`
