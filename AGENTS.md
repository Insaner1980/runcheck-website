...
- Published article locales are `en`, `fi`, `sv`, `nb`, `de`, `da`, `fr`, and `es`. Every available translation is paired by `sourceNumber`; detail pages emit reciprocal `hreflang` links only for routed versions, while English remains `x-default`.
...
- Italian `it` content is source-supported and may be built locally only with `RUNCHECK_PREVIEW_ARTICLE_LOCALES=it npm run build` (PowerShell: `$env:RUNCHECK_PREVIEW_ARTICLE_LOCALES='it'; npm run build`). It remains `productionPublished: false`, uses `noindex, nofollow` in preview, is filtered from the sitemap, and is absent from default production routes and hreflang groups until a separate explicit gate change.
...
- Article frontmatter schema:
  ...
  - `subgroup?: string`
  - `tags: string[]`
- `locale` is validated from the source-supported locale list in `src/data/articleLocaleConfig.mjs` (currently `en`, `fi`, `sv`, `nb`, `de`, `da`, `fr`, `es`, and preview-only `it`; defaults to `en`)
  - `draft: boolean`
