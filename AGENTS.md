...
- Google Analytics 4 has one source of truth in `src/data/analytics.mjs`. `src/layouts/BaseLayout.astro` loads the Google tag once per page head from that module; do not duplicate Measurement IDs or inline alternate analytics snippets elsewhere.
