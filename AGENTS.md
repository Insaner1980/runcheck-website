...
- Published article locales are `en`, `fi`, `sv`, `nb`, `de`, `da`, and `fr`. Every available translation is paired by `sourceNumber`; detail pages emit reciprocal `hreflang` links only for versions that exist, while English remains `x-default`.
- Swedish, Norwegian Bokmål, German, Danish, and French article routes are generated by the shared dynamic route family under `src/pages/[locale]/[articles]/`. Locale roots, search-index paths, Open Graph locales, navigation labels, localized hub paths, and the localized route list have one source of truth in `src/data/articleLocales.mjs` and `src/data/articlePaths.mjs`.
...
- Article frontmatter schema:
  ...
  - `subgroup?: string`
  - `tags: string[]`
- `locale: "en" | "fi" | "sv" | "nb" | "de" | "da" | "fr"` (defaults to `"en"` for existing English content)
  - `draft: boolean`
