...
function mapHeaders(header) {
  ...
  const duplicates = REQUIRED_COLUMNS.filter(
    (column) => mapped.filter((value) => value === column).length > 1,
  );
  if (duplicates.length > 0)
    throw new Error(`CSV maps multiple headers to: ${duplicates.join(", ")}`);
  return mapped;
}
...
export function readCsvRecords(file) {
  ...
  return readCsvObjects(file).map((record) =>
    Object.fromEntries(
...
const parseMetric = (
  ...
  name,
  rowNumber,
  { integer = false, percent = false } = {},
) => {
  const cleaned = value.replaceAll(/\s/g, "").replace(",", ".");
  const parsed = Number(
    percent && cleaned.endsWith("%") ? cleaned.slice(0, -1) : cleaned,
  );
  const normalized = percent && cleaned.endsWith("%") ? parsed / 100 : parsed;
  if (!Number.isFinite(normalized) || normalized < 0)
    throw new Error(`Row ${rowNumber}: invalid ${name}: ${value}`);
  if (integer && !Number.isInteger(normalized))
    throw new Error(`Row ${rowNumber}: ${name} must be an integer.`);
  if (percent && normalized > 1)
    throw new Error(
      `Row ${rowNumber}: CTR must be a decimal (0-1) or a percent value.`,
    );
  return normalized;
};
...
export function buildArticleRegistry({
...
export function loadTermMap(file) {
  ...
  for (const record of records) {
    const sourceExpression =
      /Important source IDs:\s*([0-9, -]+)/.exec(record.notes)?.[1] ?? "";
    const sourceNumbers = parseSourceNumberExpression(sourceExpression);
    const terms = [
      record.recommended_primary_term,
      ...record.accepted_alternatives.split(";"),
    ]
      .map((term) => term.trim())
      .filter(Boolean);
...
const detectLocaleFromUrl = (url) =>
  Object.values(ARTICLE_LOCALE_CONTRACTS)
    .filter((contract) => contract.productionPublished)
    .find((contract) => new URL(url).pathname.startsWith(contract.root))
    ?.code ?? "";
...
export function analyzeRecords(
  ...
  const normalizedRows = records.map((record, index) => {
    ...
    const clicks = parseMetric(record.clicks, "clicks", rowNumber, {
      integer: true,
    });
    ...
    const averagePosition = parseMetric(
    ...
    const sharedCoreTerms = queryTokens.filter((token) =>
      titleTokens.has(token),
    );
    const termEntries = article
      ? (termMap.get(`${article.regionalProfile}:${article.sourceNumber}`) ??
        [])
      : [];
    const validatedTerm = findValidatedTerm(record.query, termEntries);
    const queryInTitle = article
      ? normalizeText(article.title).includes(normalizeText(record.query))
      : false;
    const validatedTermInTitle = validatedTerm
      ? normalizeText(article.title).includes(normalizeText(validatedTerm))
      : false;
    const highImpressionsLowCtr =
      impressions >= minImpressions && ctr <= maxCtr;
    const positionOpportunity =
      averagePosition >= minPosition && averagePosition <= maxPosition;
    const termGap = Boolean(
      article &&
      !queryInTitle &&
  ...
  const opportunities = normalizedRows
    .filter(
      (row) =>
        row.highImpressionsLowCtr || row.positionOpportunity || row.termGap,
    )
    .map((row) => ({
      page: row.page,
  ...
  const cannibalization = [
    ...
    .map((group) => ({
      ...
      sourceNumbers: [...new Set(group.map((row) => row.article.sourceNumber))]
        .sort((a, b) => a - b)
        .join("; "),
      ...
      end_date: group
        ...
        .at(-1),
      evidence:
        "The same normalized query appears for multiple URLs in the same locale in the imported rows.",
      recommended_action:
        "Inspect page × query history and intent before changing, merging, redirecting, or canonicalizing pages.",
    }));
  ...
  const countryMismatch = normalizedRows
    .filter((row) => {
      ...
      return (
        expected &&
        row.impressions >= minCountryImpressions &&
        !expected.has(row.country.trim().toUpperCase())
      );
    })
    .map((row) => ({
      page: row.page,
      ...
      evidence: row.article
        ? "Imported GSC row mapped to a published localized article."
        : "Locale path detected, but URL is not mapped to a current article.",
    }));
  ...
  return {
    ...
    unknownUrls: [
      ...new Set(
        normalizedRows.filter((row) => !row.article).map((row) => row.page),
      ),
    ],
  };
}
...
export function runAnalysis({
  ...
  if (minPosition > maxPosition)
    throw new Error("minPosition must not exceed maxPosition.");
  ...
  writeCsv(
    path.join(outputDir, OUTPUT_FILES.opportunities),
    [
      "page",
      "sourceNumber",
      "locale",
      ...
      "position_range_candidate",
      "query_exactly_in_title_h1",
      "shared_query_title_core_terms",
      "matched_validated_term",
      "validated_term_in_title_h1",
      "validated_term_gap",
      "evidence_level",
      "recommended_action",
    ],
  ...
  writeCsv(
    path.join(outputDir, OUTPUT_FILES.cannibalization),
    [
      "locale",
      "query",
      "urls",
  ...
  writeCsv(
    path.join(outputDir, OUTPUT_FILES.countryMismatch),
    [
      "page",
      "sourceNumber",
      "locale",
  ...
  writeSummary(
    path.join(outputDir, OUTPUT_FILES.summary),
    analysis,
    thresholds,
  );
  return analysis;
