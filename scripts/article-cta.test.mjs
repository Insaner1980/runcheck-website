...
test('article cta', () => {
  ...
  assert.match(articleCta, /href=\{ARTICLE_LOCALES\[locale\]\.root\}/, 'ArticleCTA secondary action should use the central locale root.');
  ...
  assert.match(footer, /href="\/#features">\{copy\.nav\[0\]\}<\/a>/, 'Footer should expose the localized product feature path sitewide.');
