...
assert.match(articleCta, /href="\/articles\/"/, 'ArticleCTA secondary action should point to the article index.');
...
assert.match(articleIndex, /<ArticleCTA\s+includeGuidesLink=\{false\}\s*\/>/, 'Articles index should not render a self-linking guide CTA action.');
...
assert.match(footer, /href="\/#features">Features<\/a>/, 'Footer should expose the product feature path sitewide.');
