...
test("tuotannon locale-portti julkaisee yhdeksän localea, myös Italian", () => {
  assert.deepEqual(PUBLISHED_ARTICLE_LOCALE_CODES, [
    "en",
    "fi",
    "sv",
    "nb",
  ...
  assert.deepEqual(LOCALIZED_ARTICLE_LOCALES, [
    "sv",
    "nb",
    "de",
    "da",
  ...
  assert.equal(
    isProductionSitemapUrl("https://runcheckapp.com/it/articoli/"),
    true,
  );
  assert.equal(
    isProductionSitemapUrl("https://runcheckapp.com/es/articulos/"),
    true,
  );
  ...
  for (const contract of Object.values(ARTICLE_LOCALE_CONTRACTS)) {
    for (const field of [
      ...
      assert.notEqual(
        contract[field],
        undefined,
        `${contract.code}.${field} puuttuu locale-sopimuksesta.`,
      );
    }
    if (contract.code !== "en")
      assert.equal(Object.keys(contract.hubSlugs).length, 13);
  }
});

test("jo julkaistua Italiaa ei voi pyytää preview-localeksi", () => {
  const script =
    "import('./src/data/articleLocaleConfig.mjs').then(m => process.stdout.write(JSON.stringify(m.ROUTED_ARTICLE_LOCALE_CODES)))";
  assert.throws(
    () =>
