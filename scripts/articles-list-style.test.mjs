...
test("articles list style", () => {
  ...
  for (const [name, file] of files) {
    ...
    assert.doesNotMatch(
      source,
      /Article \{article\.data\.sourceNumber\}/,
      `${name} should not show per-article numbers or metadata.`,
    );
    assert.doesNotMatch(
      source,
      /<li class="[^"]*\b(?:rounded-panel|border|bg-run-surface|shadow-panel|p-5)\b[^"]*"/,
      `${name} should not add per-row heavy card utilities.`,
    );
    assert.match(
      source,
  ...
  const cssBlockFor = (selector) => {
    const matches = [
      ...globalStyles.matchAll(
        new RegExp(`${selector.replaceAll(".", "\\.")}\\s*\\{([^}]*)\\}`, "gs"),
      ),
    ];
  ...
  assert.match(
    contentConfig,
    /listSummary:\s*z\.string\(\)\.optional\(\)/,
    "Article schema should allow an optional listSummary field.",
  );
  assert.match(
    articleDetailSource,
  ...
  assert.ok(
    articleCardInteractiveBlock,
    "Article list cards should define a shared hover and focus-visible block.",
  );
  assert.doesNotMatch(
    cssBlockFor(".article-list-link"),
    /transform:/,
    "Article list cards should not move in their base state.",
  );
  assert.doesNotMatch(
    articleCardInteractiveBlock,
    /transform:/,
    "Article list cards should stay simple without hover movement.",
  );
  assert.match(
    cssBlockFor(".article-link-list"),
    /grid-template-columns:\s*repeat\(auto-fit,\s*minmax\(min\(100%,\s*20rem\),\s*1fr\)\);/,
    "Article lists should use responsive short tile columns.",
  );
  assert.match(
    cssBlockFor(".article-link-list"),
    /gap:\s*0\.9rem;/,
    "Article lists should use compact gaps between lightweight tiles.",
  );
  assert.doesNotMatch(
    cssBlockFor(".article-link-list"),
    /border-top:/,
    "Article card lists should not keep the old top divider.",
  );
  assert.match(
    cssBlockFor(".article-list-link"),
    /border:\s*1px solid rgb\(255 255 255 \/ 0\.09\);/,
    "Article tiles should keep the current subtle border.",
  );
  assert.doesNotMatch(
    cssBlockFor(".article-list-link"),
    /border-radius:/,
    "Article tiles should keep the current square treatment.",
  );
  assert.match(
    cssBlockFor(".article-list-link"),
  ...
  assert.doesNotMatch(
    cssBlockFor(".article-list-link"),
    /box-shadow:/,
    "Article cards should avoid heavy shadows.",
  );
  assert.match(
    cssBlockFor(".article-list-link"),
    /padding:\s*var\(--run-article-card-padding\);/,
    "Article links should use centralized compact card padding.",
  );
  assert.match(
    cssBlockFor(".article-list-link"),
    /min-height:\s*7rem;/,
    "Article links should render as short rectangular tiles.",
  );
  assert.doesNotMatch(
    cssBlockFor(".article-list-description"),
    /white-space:\s*nowrap;/,
    "Article descriptions should be allowed to wrap inside cards.",
  );
  assert.doesNotMatch(
    cssBlockFor(".article-link-list > li"),
    /border-bottom:/,
    "Article card list items should not keep old row dividers.",
  );
  assert.match(
    cssBlockFor(".article-list-link"),
    /min-width:\s*0;/,
    "Article links should be allowed to shrink inside the list column.",
  );
  assert.match(
    cssBlockFor(".article-link-list > li"),
    /min-width:\s*0;/,
    "Article list grid items should not overflow from nowrap summaries.",
  );
});
