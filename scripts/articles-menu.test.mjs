...
test("articles menu", () => {
  ...
  assert.doesNotMatch(
    header,
    /Suomeksi|label: isFinnish \? 'English'/,
    "Header should not show a target-language switch link.",
  );
  assert.doesNotMatch(
    header,
    /data-articles-menu|ARTICLE_MENU_COLUMNS|articles-menu-panel/,
    "Header should not retain the old article mega-menu.",
  );
  assert.match(
    header,
  ...
  assert.match(
    ...
    "Mobile navigation should expose a localized open label.",
  );
  assert.match(
    header,
    /data-close-label=\{menuLabels\.close\}/,
    "Mobile navigation should expose a localized close label.",
  );
  assert.match(
    header,
  ...
  assert.doesNotMatch(
    globalStyles,
    /\.articles-menu/,
    "Dead mega-menu styles should be removed.",
  );
  assert.match(
    globalStyles,
    /\.hero-nav-action\s*\{/,
    "Shared hero nav action styles should remain centralized in global CSS.",
  );
