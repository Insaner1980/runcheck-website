...
const uniqueTerms = (...values) =>
  [...new Set(values.flatMap(splitTerms))].join(" | ");
...
export function buildKeywordDemandInput(records) {
  return records
    ...
    .map((record) => ({
      ...
      terms_to_compare: uniqueTerms(
        record.current_primary_term,
      ...
      decision_rule:
        "No title, H1, URL, slug, or body change unless demand, intent, and locale-naturalness agree.",
    }));
}
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  ...
  console.log(
    `Created ${result.rows} keyword-demand validation rows: ${result.output}`,
  );
}
