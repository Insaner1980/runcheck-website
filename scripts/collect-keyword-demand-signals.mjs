...
const defaultInput = path.join(
  root,
  "reports",
  "keyword-demand-validation-input.csv",
);
const defaultOutput = path.join(
  root,
  "reports",
  "keyword-demand-autocomplete-signals.csv",
);
...
const expandTerms = (record) =>
  [
    ...new Set(
      record.terms_to_compare
        .split(" | ")
        .map((term) => term.trim())
        .filter(Boolean),
    ),
...
export async function collectAutocompleteSignals(
  records,
  { fetchImpl = fetch } = {},
) {
      ...
      for (let index = workerIndex; index < expanded.length; index += 4) {
        ...
        try {
          ...
          results[index] = {
            ...
            exact_term_suggested: suggestions.some(
              (value) =>
                value.toLocaleLowerCase() === record.term.toLocaleLowerCase(),
            )
              ? "YES"
              : "NO",
            suggestions: suggestions.join(" | "),
            interpretation:
              "Support signal only; not search volume, ranking, CTR, or proof that a term should replace another term.",
          };
        } catch (error) {
          results[index] = {
            topic_cluster: record.topic_cluster,
...
export async function writeAutocompleteSignals(
  input = defaultInput,
  ...
  return {
    ...
    unavailable: records.filter(
      ({ autocomplete_status: status }) => status !== "OK",
    ).length,
  };
}
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  ...
  const input =
    inputIndex >= 0 ? path.resolve(process.argv[inputIndex + 1]) : defaultInput;
  const output =
    outputIndex >= 0
  ...
  console.log(
    `Created ${result.rows} autocomplete evidence rows (${result.unavailable} unavailable): ${result.output}`,
  );
}
