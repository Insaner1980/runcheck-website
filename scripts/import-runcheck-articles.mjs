...
const localeArg = process.argv.find((argument) =>
...
const generatedRoot =
  locale === "en" ? contentRoot : path.join(contentRoot, locale);
const workspaceRoot = path.resolve(".");
const FINNISH_DESCRIPTION_OVERRIDES = new Map([
  [
    68,
    "Miksi Android-puhelin voi tuntua hitaalta päivityksen jälkeen, milloin se on normaalia ja mitä tehdä, jos suorituskyky ei palaudu.",
  ],
  [
    69,
    "Mitkä Androidin Developer Options -asetukset voivat saada puhelimen tuntumaan nopeammalta, mitkä ovat riskialttiita ja mitkä kannattaa jättää rauhaan.",
  ],
]);
...
const firstLongParagraph = (body, excludedText = "") =>
  body
    ...
    .find(
      (part) =>
        part.length >= 70 &&
        normalizedText(part) !== normalizedText(excludedText),
    ) ?? "";
...
const buildLocalizedListSummary = (description, title) => {
  ...
  const candidate =
    sentences.find(
      (sentence) => normalizedText(sentence) !== normalizedText(title),
    ) ?? clean;
...
const localizedHubFor = (hub, articleLocale) => {
  ...
  return localizedHubs(articleLocale).find(
...
const findSectionTitle = (hub, sourceNumber) => {
  const section = hub.sections?.find((candidate) =>
    candidate.sourceNumbers.includes(sourceNumber),
  );
...
const formatFrontmatter = (data) =>
  [
    ...
    ...(data.metaDescription
      ...
      : []),
    ...(data.listSummary
      ? [`listSummary: ${JSON.stringify(data.listSummary)}`]
      : []),
...
async function collectSourceFiles() {
  ...
  const candidates = getMarkdownFiles(sourceRoot).sort((a, b) =>
    a.localeCompare(b),
  );
  ...
  for (const file of candidates) {
    ...
    if (existing)
      throw new Error(
        `Article ${sourceNumber} has duplicate translations in ${sourceRoot}: ${existing}; ${file}`,
      );
  ...
  if (candidates.length !== 160)
    throw new Error(
      `Expected exactly 160 source files for ${locale}, found ${candidates.length}.`,
    );
...
function assertCompleteSourceSet(sourceByNumber) {
  ...
  const actualMissing = expectedNumbers.filter(
    (number) => !sourceByNumber.has(number),
  );
  if (actualMissing.length > 0) {
    throw new Error(
      `Missing article numbers for ${locale}: ${actualMissing.join(",")}.`,
    );
  }
}
...
async function readExistingArticleMetadata() {
  ...
  for (const generatedFile of getMarkdownFiles(generatedRoot)) {
    ...
    const sourceNumber = Number(
      parseFrontmatterString(frontmatter, "sourceNumber"),
    );
    const existingLocale =
      parseFrontmatterString(frontmatter, "locale") || "en";
...
async function prepareGeneratedRoot() {
  ...
  if (locale !== "en") {
    ...
    await Promise.all(
      ARTICLE_HUBS.map((hub) =>
        rm(path.join(generatedRoot, hub.slug), {
          recursive: true,
          force: true,
        }),
      ),
  ...
  await Promise.all(
    ARTICLE_HUBS.map((hub) =>
      mkdir(path.join(generatedRoot, hub.slug), { recursive: true }),
    ),
  );
}
...
async function generateArticle(
  ...
  const extractedTitle =
    parseFrontmatterString(frontmatter, "title") ||
    extractTitle(body, sourceName);
  const title =
    locale === "fi" && sourceNumber === 113
  ...
  const explicitDescription = parseFrontmatterString(
    frontmatter,
    "description",
  );
  const sourceMetaDescription = parseFrontmatterString(
    frontmatter,
    "metaDescription",
  );
  ...
  const preservedMetaDescription =
    locale === "fi"
      ? sourceMetaDescription
      : sourceMetaDescription || existingMetadata.metaDescription || "";
  const withoutTitle = removeFirstH1(body);
  const meta = extractMetaDescription(withoutTitle);
  const sourceDescription =
    explicitDescription || meta.description || firstParagraph(meta.body);
  const description =
    locale === "fi"
  ...
  const metaDescription =
    preservedMetaDescription ||
    (locale === "fi" && description.length < 70
      ? firstLongParagraph(meta.body, description)
      : "");

  if (!description) {
    throw new Error(
      `${sourceName} does not contain enough text to derive a description.`,
    );
  }

  const bodyWithoutDuplicateDescription = removeLeadingDuplicateDescription(
    meta.body,
    sourceDescription,
  );
  const slug =
...
async function importArticles() {
  ...
  const sourceFiles = [...sourceByNumber.values()].sort(
