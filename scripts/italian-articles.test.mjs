...
const sourceRoot =
  "C:/Users/emmah/Documents/ObsidianVault/Projects/Active/runcheck/artikkelit/it-IT";
const markdownFiles = (root) =>
  readdirSync(root, { withFileTypes: true }).flatMap((entry) =>
    entry.isDirectory()
      ? markdownFiles(path.join(root, entry.name))
...
test("Italian 160 preview-artikkelia täyttävät metadata-, slug- ja kieliportin", (t) => {
  ...
  const englishWords = new Set(
    "the and to of is are that with for you your phone battery can if on in from or this when how what why does should".split(
      " ",
    ),
  );
  const italianWords = new Set(
    "il lo la i gli le e di che con per tu tuo telefono batteria può se su in da o questo quando come cosa perché".split(
      " ",
    ),
  );
  ...
  for (const file of files) {
    ...
    assert.ok(
      typeof data.listSummary === "string" &&
    ...
    assert.ok(
      !routeSlugs.has(routeKey),
      `${routeKey} törmää toiseen Italian reittiin.`,
    );
    assert.doesNotMatch(data.listSummary, /[·—]/);
    assert.notEqual(
      data.listSummary,
      new Intl.ListFormat("it", { style: "long", type: "conjunction" }).format(
        data.tags.slice(0, 3),
      ),
      `ID ${data.sourceNumber}: listSummary ei saa olla tagiluettelo.`,
    );
    ...
    for (const paragraph of body.split(/\r?\n\s*\r?\n/)) {
      ...
      const englishScore = new Set(
        words.filter((word) => englishWords.has(word)),
      ).size;
      const italianScore = new Set(
        words.filter((word) => italianWords.has(word)),
      ).size;
      if (englishScore >= 7 && englishScore > italianScore * 1.5)
        suspiciousEnglish.push({ file, paragraph });
    }
  ...
  assert.doesNotMatch(
    corpus,
    /salute della batteria|punteggio di salute|capacità di progetto|prosenttia/,
  );
});
