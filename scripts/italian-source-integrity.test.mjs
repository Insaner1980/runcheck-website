...
const SOURCE_BASE =
  "C:/Users/emmah/Documents/ObsidianVault/Projects/Active/runcheck/artikkelit";
const roots = {
  en: path.join(SOURCE_BASE, "Valmiit"),
  it: path.join(SOURCE_BASE, "it-IT"),
};
...
const sourceNumber = (file) =>
  Number(path.basename(file).match(/^(\d+)-/)?.[1]);
const bySourceNumber = (root) =>
  new Map(markdownFiles(root).map((file) => [sourceNumber(file), file]));
const readUtf8 = (file) => utf8.decode(readFileSync(file));
const headingLevels = (source) =>
  [...source.matchAll(/^(#{1,6})\s+/gm)].map((match) => match[1].length);
const linkTargets = (source) =>
  [...source.matchAll(/\[[^\]]*\]\(([^)]+)\)/g)]
    .map((match) => match[1])
    .sort();
const canonicalFilenameStem = (file) => {
  let stem = decodeURIComponent(path.basename(file, ".md"))
    .replace(/^\d+-/, "")
    .replace(/-it-IT$/i, "");
  while (/\(\d+\)(?:\(\d+\))?$/.test(stem))
    stem = stem.replace(/\(\d+\)(?:\(\d+\))?$/, "");
  return stem;
};
...
test("Italian lähdeaineisto on eheä ja vastaa ID-kohtaisia englanninkielisiä lähteitä", () => {
  ...
  assert.deepEqual(
    [...italianById.keys()].sort((a, b) => a - b),
    expectedIds,
  );
  assert.deepEqual(
    [...englishById.keys()].sort((a, b) => a - b),
    expectedIds,
  );
  ...
  for (const id of expectedIds) {
    ...
    assert.equal(
      canonicalFilenameStem(italianFile),
      canonicalFilenameStem(englishFile),
      `ID ${id}: tiedostonimi ei vastaa englanninkielistä lähdettä.`,
    );
    assert.match(italian, /^#\s+\S.+$/m, `ID ${id}: Italian H1 puuttuu.`);
    assert.deepEqual(
      headingLevels(italian),
      headingLevels(english),
      `ID ${id}: otsikkotasot eivät vastaa lähdettä.`,
    );
    assert.deepEqual(
      linkTargets(italian),
      linkTargets(english),
      `ID ${id}: Markdown-linkkien kohteet poikkeavat lähteestä.`,
    );
  ...
  assert.equal(
    absentFrontmatter,
    1,
    "ID 1 käyttää sallittua lähde-Markdownia ilman lähdefrontmatteria; generoitu tiedosto saa täydellisen frontmatterin importerilta.",
  );
});
