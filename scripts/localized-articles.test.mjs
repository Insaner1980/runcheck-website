...
test("localized article content and routes", () => {
  ...
  for (const [locale, expected] of Object.entries(expectedCounts)) {
    ...
    assert.equal(
      files.length,
      expected,
      `${locale} should contain the expected number of available translations.`,
    );
    ...
    for (const file of files) {
      ...
      assert.ok(
        ...
        `${file} should declare locale ${locale}.`,
      );
      assert.doesNotMatch(
        path.basename(file),
        /^\d+-/,
        `${locale} filenames should be localized SEO slugs.`,
      );
  ...
  assert.ok(
    !readFileSync(markdownFiles(path.join(root, "fi"))[0], "utf8").includes(
      'tags: ["battery"',
    ),
    "Finnish search tags should be localized.",
  );

  const routeChecks = [
    ["sv/artiklar/index.html", 'lang="sv"'],
    ["nb/artikler/index.html", 'lang="nb"'],
    ["de/artikel/index.html", 'lang="de"'],
    ["da/artikler/index.html", 'lang="da"'],
    ["fr/articles/index.html", 'lang="fr"'],
    ["es/articulos/index.html", 'lang="es"'],
    ["sv/artiklar/sokindex.json", '"url":"/sv/artiklar/'],
    ["nb/artikler/sokeindeks.json", '"url":"/nb/artikler/'],
    ["de/artikel/suchindex.json", '"url":"/de/artikel/'],
    ["da/artikler/soegeindeks.json", '"url":"/da/artikler/'],
    ["fr/articles/index-recherche.json", '"url":"/fr/articles/'],
    ["es/articulos/indice-busqueda.json", '"url":"/es/articulos/'],
  ];
  for (const [relative, marker] of routeChecks)
    assert.ok(
      readFileSync(path.resolve("dist", relative), "utf8").includes(marker),
      `${relative} should be generated for its locale.`,
    );
