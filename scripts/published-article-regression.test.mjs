...
const outputFile = (urlPath) =>
  path.resolve("dist", urlPath.replace(/^\//, ""), "index.html");
...
test("kaikki 1 440 julkaistua artikkelia säilyttävät teknisen SEO-sopimuksen", () => {
  ...
  const articles = markdownFiles(contentRoot)
    .map((file) => {
      const source = readFileSync(file, "utf8");
      return {
        file,
        source,
        locale: frontmatterValue(source, "locale") ?? "en",
      };
    })
    .filter((article) => published.has(article.locale));
  const sitemap = readFileSync("dist/sitemap-0.xml", "utf8");
  const byLocale = Object.fromEntries(
    PUBLISHED_ARTICLE_LOCALE_CODES.map((locale) => [locale, 0]),
  );

  assert.equal(articles.length, 1440);
  assert.ok(
    existsSync("dist/it/articoli"),
    "Tuotantobuildista puuttuvat Italian artikkelireitit.",
  );
  ...
  for (const article of articles) {
    ...
    assert.ok(
      html.includes(
        `<link rel="canonical" href="https://runcheckapp.com${urlPath}"`,
      ),
      `${urlPath}: canonical on väärä.`,
    );
    assert.ok(
      html.includes(`>${htmlEscape(title)}</h1>`),
      `${urlPath}: H1 ei vastaa frontmatter-titleä.`,
    );
    for (const locale of PUBLISHED_ARTICLE_LOCALE_CODES)
      assert.ok(
        html.includes(`hreflang="${locale}"`),
        `${urlPath}: hreflang ${locale} puuttuu.`,
      );
    assert.ok(html.includes('hreflang="x-default"'));
    assert.doesNotMatch(html, /<meta name="robots" content="noindex/);
    assert.ok(
      sitemap.includes(`<loc>https://runcheckapp.com${urlPath}</loc>`),
      `${urlPath}: sitemap-merkintä puuttuu.`,
    );
  ...
  assert.deepEqual(
    byLocale,
    Object.fromEntries(
      PUBLISHED_ARTICLE_LOCALE_CODES.map((locale) => [locale, 160]),
    ),
  );
  for (const migration of ARTICLE_SLUG_MIGRATIONS) {
    assert.ok(
      sitemap.includes(
        `<loc>https://runcheckapp.com${migration.newPath}</loc>`,
      ),
    );
    assert.ok(
      !sitemap.includes(
        `<loc>https://runcheckapp.com${migration.oldPath}</loc>`,
      ),
    );
  }
});
