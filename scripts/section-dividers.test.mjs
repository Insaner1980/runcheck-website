...
test("section dividers", () => {
  ...
  assert.match(
    styles,
    /\.features-layout\s*\{[^}]*display:\s*grid/s,
    "Feature section should keep the current grid layout.",
  );
  assert.match(
    styles,
    /\.feature-item\s*\{[^}]*display:\s*grid/s,
    "Feature items should keep their current structured layout.",
  );
  assert.match(
    styles,
    /\.pricing-trial-divider\s*\{[^}]*height:\s*1px/s,
    "Pro trial divider should remain.",
  );
  assert.match(
    styles,
    /\.pricing-plan-pro\s*\{[^}]*border:\s*1px solid var\(--run-pricing-pro-edge\)/s,
    "Pro pricing boundary should remain tokenized.",
  );
  assert.match(
    styles,
    /\.faq-item\s*\{[^}]*border-bottom:\s*1px solid var\(--run-faq-divider\)/s,
    "FAQ item dividers should remain tokenized.",
  );
});
