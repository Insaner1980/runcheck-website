...
const expectedTitles = new Map([
  ...
  [
    "fr:123",
    "Votre téléphone est-il encore pris en charge ? Vérifier la durée du suivi logiciel et des mises à jour de sécurité",
  ],
  [
    "es:123",
    "¿Sigue teniendo soporte tu móvil? Cómo comprobar el periodo de soporte y las actualizaciones de seguridad",
  ],
]);
...
const builtHtml = (article) => {
  ...
  const route = articlePath(
    hub,
    path.basename(article.file, ".md"),
    article.locale,
  );
  return readFileSync(
...
const articles = publishedLocalizedLocales.flatMap((locale) =>
  markdownFiles(path.join(contentRoot, locale)).map((file) => {
    const source = readFileSync(file, "utf8");
    return {
      file,
      source,
      locale,
      sourceNumber: value(source, "sourceNumber"),
    };
  }),
);
const byKey = new Map(
  articles.map((article) => [
    `${article.locale}:${article.sourceNumber}`,
    article,
  ]),
);
const tagListPattern =
  /^[^.!?]+,\s+[^.!?]+\s+(?:and|ja|och|og|und|et|y)\s+[^.!?]+$/i;
...
test("neljä REVISION-titleä ja H1:tä vastaavat hyväksyttyä CSV-ehdotusta", () => {
  ...
  const priorityReport = "reports/priority-article-keyword-review.csv";
  if (existsSync(priorityReport)) {
    assert.match(readFileSync(priorityReport, "utf8"), /REVISION RECOMMENDED/);
  }
  assert.match(
    byKey.get("fi:122").source,
    /^## Mistä tietoturvapäivityksen päivämäärä löytyy$/m,
  );
  assert.match(
    byKey.get("fi:122").source,
    /^## Kuinka tuore tietoturvapäivityksen pitäisi olla\?$/m,
  );
  assert.match(
    byKey.get("sv:122").source,
    /^## Hur aktuell bör säkerhetsuppdateringen vara\?$/m,
  );
  assert.match(byKey.get("fi:122").source, /Android security patch level/);
  assert.match(
    byKey.get("sv:122").source,
    /Androids säkerhetskorrigeringsnivå/,
  );
});
...
test("yhdeksän metaTitle- ja kolme metaDescription-ehdotusta on toteutettu täsmällisesti", () => {
  ...
  for (const [key, metaTitle] of expectedMetaTitles)
    assert.equal(value(byKey.get(key).source, "metaTitle"), metaTitle, key);
  for (const [key, metaDescription] of expectedMetaDescriptions)
    assert.equal(
      value(byKey.get(key).source, "metaDescription"),
      metaDescription,
      key,
    );
});
...
test("hyväksytyt H1- ja metadata-arvot näkyvät tuotantobuildin HTML:ssä", () => {
  for (const [key, title] of expectedTitles) {
    ...
    assert.ok(
      html.includes(`>${htmlEscape(title)}</h1>`),
      `${key}: H1 ei näy buildissa.`,
    );
  }
  for (const [key, metaTitle] of expectedMetaTitles) {
    const html = builtHtml(byKey.get(key));
    assert.ok(
      html.includes(`<title>${htmlEscape(metaTitle)}</title>`),
      `${key}: metaTitle ei näy buildissa.`,
    );
  }
  for (const [key, metaDescription] of expectedMetaDescriptions) {
    const html = builtHtml(byKey.get(key));
    assert.ok(
      ...
      `${key}: metaDescription ei näy buildissa.`,
    );
  }
});

test("listSummary-pilotti ja lähde-esteiden korjaukset jättävät 506 tagiluetteloa myöhemmäksi", () => {
  ...
  for (const key of pilotKeys) {
    ...
    assert.ok(
      summary.length <= 110,
      `${key}: listSummary on liian pitkä (${summary.length}).`,
    );
    if (article.locale === "fi")
      assert.ok(
        summary.length <= 72,
        `${key}: suomenkielinen listSummary on liian pitkä.`,
      );
    assert.doesNotMatch(
      summary,
      tagListPattern,
      `${key}: listSummary on edelleen tagiluettelo.`,
    );
    assert.doesNotMatch(
      summary,
      /[·—]/,
      `${key}: listSummary sisältää kielletyn erottimen.`,
    );
  }
  ...
  assert.equal(remainingTagLists.length, 506);
  assert.deepEqual(
    Object.fromEntries(
    ...
    { fi: 66, sv: 65, nb: 61, da: 69, de: 76, fr: 89, es: 80 },
  );
});
...
test("Italian source 123 säilyy preview-aineistona ilman Codexin natiivitarkistusväitettä", () => {
  ...
  assert.equal(
    value(source, "title"),
    "Il mio smartphone è ancora supportato? Come verificare il ciclo degli aggiornamenti e del supporto di sicurezza Android",
  );
  assert.equal(
    path.basename(file, ".md"),
    "il-mio-smartphone-e-ancora-supportato-come-verificare-il-ciclo-degli-aggiornamenti-e-del",
  );
...
test("prioriteettipilotin locale-profiilit pysyvät yksiselitteisinä", () => {
  assert.deepEqual(Object.values(profileByLocale).sort(), [
    "da-DK",
    "de-DE",
    "es-ES",
    "fi-FI",
