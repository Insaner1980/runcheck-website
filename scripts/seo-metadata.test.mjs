...
test("seo metadata", () => {
  ...
  function tags(head, name) {
    return [...head.matchAll(new RegExp(`<${name}\\b[^>]*>`, "gi"))].map(
      (match) => ({
        tag: match[0],
  ...
  assert.ok(
    existsSync(root),
    "dist should exist; run npm run build before the SEO metadata test.",
  );
    ...
    for (const entry of readdirSync(dir, { withFileTypes: true })) {
      ...
      if (entry.isDirectory()) {
        ...
        const locale = JSON.parse(
          source.match(/^locale: (.+)$/m)?.[1] ?? '"en"',
        );
        const hub = JSON.parse(source.match(/^hub: (.+)$/m)?.[1]);
        const sourceNumber = Number(
          source.match(/^sourceNumber: (\d+)$/m)?.[1],
        );
        const url = new URL(
          articlePath(hub, path.basename(fullPath, ".md"), locale),
          site,
        ).toString();
  ...
  assert.ok(
    htmlFiles.length > 0,
    "The built site should expose static HTML pages.",
  );
  ...
  assert.equal(
    GA4_MEASUREMENT_ID,
    "G-4GJKG2Z3ZL",
    "GA4 measurement ID should match the configured runcheck web stream.",
  );
  assert.equal(
    CLOUDFLARE_WEB_ANALYTICS_TOKEN,
  ...
  function parseCanonicalSiteUrl(publicUrl, context) {
    ...
    assert.equal(
      parsed.origin,
      site,
      `${context} should stay on the canonical site.`,
    );
    return parsed;
  }
  ...
  assert.throws(
    () =>
      parseCanonicalSiteUrl(
        "https://runcheckapp.com.example/runcheck-search-thumbnail.webp",
        "lookalike image URL",
      ),
    /canonical site/,
  ...
  function assertWebpImageUrl(imageUrl, context) {
    ...
    assert.equal(
      ...
      ".webp",
      `${context} should use a WebP image.`,
    );
  }
  ...
  for (const file of htmlFiles) {
    ...
    const expectedLocale =
      Object.values(ARTICLE_LOCALE_CONTRACTS).find(
        (item) => item.code !== "en" && pathname.startsWith(item.root),
      ) ?? ARTICLE_LOCALE_CONTRACTS.en;
    ...
    const metaByName = (name) =>
      meta.find((item) => item.attrs.name === name)?.attrs.content ?? "";
    const metaByProp = (property) =>
      meta.find((item) => item.attrs.property === property)?.attrs.content ??
      "";
    ...
    const duplicateSourcePair = duplicateTitleUrl
      ? new Set([
          articleSourceNumberByUrl.get(url),
          articleSourceNumberByUrl.get(duplicateTitleUrl),
        ])
      : new Set();
    assert.ok(
      !duplicateTitleUrl ||
        (duplicateSourcePair.size === 2 &&
          duplicateSourcePair.has(112) &&
          duplicateSourcePair.has(113)),
      `${url} title duplicates ${duplicateTitleUrl}.`,
    );
    ...
    if (description.length < 70)
      descriptionWarnings.push(`${url} (${description.length})`);
    assert.ok(
      description.length <= 180,
      `${url} meta description should stay concise.`,
    );
    const descriptionKey = `${expectedLanguage}:${description}`;
    if (seenDescriptions.has(descriptionKey))
      duplicateDescriptionWarnings.push(
        `${url} duplicates ${seenDescriptions.get(descriptionKey)}`,
      );
    ...
    assert.equal(
      metaByName("theme-color"),
      "#030708",
      `${url} should expose the browser theme color.`,
    );
    assert.equal(
      canonicalTags.length,
      1,
      `${url} should have exactly one canonical link.`,
    );
    assert.equal(
      canonicalTags[0].attrs.href,
      url,
      `${url} canonical should match the built URL.`,
    );
    assert.equal(
      html.match(/<html\b[^>]*\blang="([^"]+)"/i)?.[1],
      expectedLanguage,
      `${url} should expose the correct HTML language.`,
    );
    assert.equal(
      metaByProp("og:locale"),
      expectedLocale.og,
      `${url} should expose the correct Open Graph locale.`,
    );
    assert.equal(
      metaByProp("og:url"),
      url,
      `${url} og:url should match canonical.`,
    );
    assert.equal(
      link.find((item) => item.attrs.rel === "manifest")?.attrs.href,
      "/site.webmanifest",
      `${url} should link the web manifest.`,
    );
    assert.equal(
      link.find((item) => item.attrs.rel === "apple-touch-icon")?.attrs.href,
      "/apple-touch-icon.png",
      `${url} should link the Apple touch icon.`,
    );
    const googleTagScripts = scriptTags.filter((item) =>
      item.attrs.src?.startsWith("https://www.googletagmanager.com/gtag/js"),
    );
    assert.equal(
      googleTagScripts.length,
      1,
      `${url} should load the Google tag exactly once.`,
    );
    assert.match(
      googleTagScripts[0].tag,
      /\sasync(?:\s|>|=)/,
      `${url} Google tag script should load asynchronously.`,
    );
    assert.equal(
    ...
    assert.match(
      head,
      new RegExp(`ga4MeasurementId\\s*=\\s*["']${GA4_MEASUREMENT_ID}["']`),
      `${url} should inline the GA4 measurement ID.`,
    );
    assert.match(
      head,
      /window\.gtag\(["']config["']\s*,\s*ga4MeasurementId\)/,
      `${url} should configure GA4 once the tag is loaded.`,
    );
    const cloudflareBeaconScripts = scriptTags.filter(
      (item) => item.attrs.src === CLOUDFLARE_WEB_ANALYTICS_SCRIPT_URL,
    );
    assert.equal(
      cloudflareBeaconScripts.length,
      1,
      `${url} should load Cloudflare Web Analytics exactly once.`,
    );
    assert.match(
      cloudflareBeaconScripts[0].tag,
      /\sdefer(?:\s|>|=)/,
      `${url} Cloudflare Web Analytics should defer loading.`,
    );
    assert.ok(
    ...
    assert.equal(
      metaByProp("og:image:type"),
      "image/webp",
      `${url} should expose the Open Graph image type.`,
    );
    assert.equal(
      metaByProp("og:image:width"),
      "1200",
      `${url} should expose the Open Graph image width.`,
    );
    assert.equal(
      metaByProp("og:image:height"),
      "1200",
      `${url} should expose the Open Graph image height.`,
    );
    ...
    assert.equal(
      ...
      metaByProp("og:image"),
      `${url} Twitter image should match the Open Graph image.`,
    );
    assert.ok(
      existsSync(distPathForPublicUrl(metaByProp("og:image"))),
      `${url} Open Graph image should resolve in dist.`,
    );
    ...
    assert.ok(
      ...
      `${url} should include WebSite JSON-LD.`,
    );
    assert.ok(
      structuredTypes.has("Organization"),
      `${url} should include Organization JSON-LD.`,
    );
    assert.ok(
      structuredTypes.has("WebPage"),
      `${url} should include WebPage JSON-LD.`,
    );
    const webPageJsonLd = structuredData.find(
      (item) => item["@type"] === "WebPage",
    );
    assert.equal(
      webPageJsonLd.inLanguage,
      expectedLanguage,
      `${url} WebPage JSON-LD should match the rendered language.`,
    );
    assert.equal(
    ...
    if (url === `${site}/`) {
      const appJsonLd = structuredData.find((item) => {
        ...
        return (
          ...
          type.includes("MobileApplication")
        );
      });

      assert.ok(
        appJsonLd,
        `${url} should include runcheck SoftwareApplication JSON-LD.`,
      );
      assert.equal(
        appJsonLd.name,
        "runcheck",
        `${url} SoftwareApplication should name the app.`,
      );
      assert.equal(
        appJsonLd.image,
        `${site}/runcheck-search-thumbnail.webp`,
      ...
      assert.equal(
        appJsonLd.operatingSystem,
        "Android",
        `${url} SoftwareApplication should identify Android as the OS.`,
      );
      assert.equal(
      ...
      assert.ok(
        !("aggregateRating" in appJsonLd),
        `${url} SoftwareApplication should not invent aggregate ratings.`,
      );
      assert.ok(
        !("review" in appJsonLd),
        `${url} SoftwareApplication should not invent reviews.`,
      );
      const expectedOffers = getStructuredOffers({ site });
      const expectedOfferCount = Object.keys(PRICING_TIERS).filter(
        (tier) => tier !== FALLBACK_PRICING_TIER,
      ).length;
      assert.equal(
        appJsonLd.offers.length,
        expectedOfferCount,
        `${url} SoftwareApplication offers should match regional Pro prices.`,
      );
      assert.equal(
        appJsonLd.offers.length,
        expectedOffers.length,
        `${url} SoftwareApplication offers should use the shared structured offer data.`,
      );
      ...
      for (const expectedOffer of expectedOffers) {
        ...
        assert.ok(
          offer,
          `${url} SoftwareApplication should include an offer for ${expectedOffer.name}.`,
        );
        assert.equal(
          offer.url,
          `${site}/#pricing`,
          `${url} ${expectedOffer.name} offer should point to the pricing section.`,
        );
        assert.deepEqual(
      ...
      assert.ok(
        !appJsonLd.offers.some((offer) => offer.name.endsWith("Default")),
        `${url} SoftwareApplication should not expose the fallback display tier as a structured offer.`,
      );
    } else {
      assert.ok(
        !structuredTypes.has("SoftwareApplication"),
        `${url} should not duplicate homepage app pricing JSON-LD.`,
      );
    }
    if (url.includes("/articles/") || url.includes("/fi/artikkelit/")) {
      assert.ok(
        structuredTypes.has("BreadcrumbList"),
        `${url} should include article breadcrumb JSON-LD.`,
      );
      assert.deepEqual(
      ...
      for (const alternate of alternateTags) {
        parseCanonicalSiteUrl(
          ...
          `${url} ${alternate.attrs.hreflang} alternate`,
        );
      }
    }
  }
  ...
  const manifest = JSON.parse(
    readFileSync(path.join(root, "site.webmanifest"), "utf8"),
  );
  ...
  assert.match(
    robots,
    /Sitemap: https:\/\/runcheckapp\.com\/sitemap-index\.xml/,
    "robots.txt should point to the sitemap index.",
  );
  assert.match(
    headers,
  ...
  assert.match(
    redirects,
    /^\/sitemap\.xml\s+\/sitemap-index\.xml\s+301$/m,
    "/sitemap.xml should redirect to the generated sitemap index.",
  );
  assert.match(
    sitemapIndex,
    /<loc>https:\/\/runcheckapp\.com\/sitemap-0\.xml<\/loc>/,
    "sitemap index should point to the generated sitemap.",
  );
  assert.equal(
    (sitemap.match(/<loc>/g) ?? []).length,
    htmlFiles.length,
    "sitemap should include every built HTML page.",
  );
  assert.equal(
    manifest.name,
    "runcheck",
    "manifest should expose the site name.",
  );
  assert.equal(
    manifest.theme_color,
    "#030708",
    "manifest should match the site theme color.",
  );

  for (const icon of manifest.icons) {
    assert.ok(
      icon.src.endsWith(".webp"),
      `manifest icon ${icon.src} should use WebP.`,
    );
    assert.equal(
      icon.type,
      "image/webp",
      `manifest icon ${icon.src} should advertise the WebP MIME type.`,
    );
  }
  ...
  for (const asset of [
    ...
    assert.ok(
      ...
      `${asset} should be copied to dist.`,
    );
  }
    ...
    JSON.stringify(
      {
        ...
        duplicateDescriptionWarningExamples: duplicateDescriptionWarnings.slice(
          0,
          8,
        ),
      },
