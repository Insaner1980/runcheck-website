...
description: "Comparison of Android diagnostic apps in 2026 including AccuBattery, DevCheck, AIDA64, CPU-Z, Device Info HW, Phone Doctor Plus, and runcheck, with what each app measures and when each one makes sense."
...
[Phone Doctor Plus](https://play.google.com/store/apps/details?id=com.idea.PhoneDoctorPlus2) says its current Android app provides more than 40 hardware and system diagnostics, including screen, speaker, microphone, camera, sensor, memory, storage, network, and battery checks. Availability and test coverage can change by device and region.
...
It brings battery, thermal, network, and storage checks into one health overview. This article does not publish a fixed weighting formula, so read the overall result together with the individual checks.
...
The free version covers battery, thermal, network, and storage diagnostics. Pro is a one-time upgrade with no subscription and adds deeper tools. The current [runcheck product page](https://runcheckapp.com/) states that diagnostics and history stay on the device and that no runcheck account is required. A user-started network test is an explicit exception: it sends test traffic to [Measurement Lab's NDT7 service](https://www.measurementlab.net/tests/ndt/ndt7/). A purchase is another explicit network boundary and is handled through [Google Play Billing](https://developer.android.com/google/play/billing). These links support separate claims; the runcheck homepage is not being used as evidence for M-Lab or Google Play behavior.
...
For overall device health, runcheck is built around a multi-category health view instead of one isolated measurement.
