...
const fitWholeWords = (words, maxLength) => {
  ...
  for (const word of words) {
    const nextLength =
      fitted.length === 0
        ? word.length
        : fitted.join("-").length + word.length + 1;
...
export function localizedSeoSlug(
  title,
  locale,
  sourceNumber,
  maxLength = MAX_ARTICLE_SLUG_LENGTH,
) {
  const override = ARTICLE_SLUG_OVERRIDES[locale]?.[sourceNumber];
  if (override) return override;

  const slugWords = SLUG_WORDS[locale] ?? SLUG_WORDS.en;
  const normalized = normalizePercentExpressions(
    title,
    locale,
    slugWords.percent,
  ...
  if (!slug)
    throw new Error(
      `Article title did not produce a slug for locale ${locale}.`,
    );
  return slug;
}
