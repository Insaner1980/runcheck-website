...
import { ARTICLE_LOCALES, localizedHubs } from '../../../data/articleLocales.mjs';
import { buildSearchIndex } from '../../../data/searchIndex.mjs';

export function getStaticPaths() {
  return ['sv', 'nb', 'de', 'da'].map((locale) => ({ params: { locale, articles: ARTICLE_LOCALES[locale].root.split('/').filter(Boolean)[1], searchIndex: ARTICLE_LOCALES[locale].searchIndex.split('/').at(-1).replace('.json', '') }, props: { locale } }));
}
